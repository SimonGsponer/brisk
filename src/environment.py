...
class Street:
    ...
    def __init__(self, engine: engine.Engine, config=config.AppConfig()) -> None:
        ...
        for i in range(config.game_config.INTERFACE_CONFIG.MAIN_FRAME_LENGTH -
                       1):
            ...
            self.street_upper_edges.insert(
                0, self.rightmost_upper_edge
            )  # .append(self.rightmost_upper_edge)
            self.street_lower_edges.insert(
                0, self.rightmost_lower_edge
            )  # .append(self.rightmost_lower_edge)

        self.graph_matrix = []
        for i in range(config.game_config.INTERFACE_CONFIG.MAIN_FRAME_WIDTH):
            self.graph_matrix.append([
                1 if ((upper_edge >= i) or (lower_edge <= i)) else 0
                for upper_edge, lower_edge in zip(self.street_upper_edges,
    ...
    def update_street(self) -> None:
        ...
        for row in range(len(self.graph_matrix)):
            ...
            if (row <= self.rightmost_upper_edge) or (
                    row >= self.rightmost_lower_edge):
    ...
    def add_obstacles(self) -> None:
        ...
        obstacles_on_upper_edge: int = self.engine.get_n_procs(proc_proba=0.5,
                                                               n_max_procs=1)

        spots_for_obstacles = self.engine.find_empty_spots_in_matrix(
            graph_matrix=np.matrix(self.graph_matrix),
            col_index=
            0  # TODO: make dynamic when allowing for changing direction
        )
