...
class TestCaseObstacle:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (pytest.param("kk", 0.5, marks=pytest.mark.xfail(raises=ValueError))),
            (pytest.param("😀", 0.5, marks=pytest.mark.xfail(raises=ValueError))),
    ...
    def test_init(self, emoji_input, proc_input):
        ...
        _ = ingame_settings.Obstacle(
            obstacle_emoji=emoji_input,
            proc_rate=proc_input
        )

    @pytest.mark.parametrize(
        "input_string,expected_len",
        [
            ("kk", 2),
            ("k", 1),
            ("☃", 1),
